...
const WORKER_LOADER_PATH = resolveMonacoPath(
...
function resolveMonacoPath(filePath) {
  return require.resolve(
    path.join('../../node_modules/monaco-editor/esm', filePath)
  );
}

const languagesById = fromPairs(
  flatMap(toPairs(LANGUAGES), ([id, language]) =>
    [id, ...(language.alias || [])].map(label => [
      label,
      { label, ...language }
    ])
  )
);
const featuresById = mapValues(FEATURES, (feature, key) => ({
  label: key,
  ...feature
}));
...
class MonacoWebpackPlugin {
  ...
  apply(compiler) {
    ...
    const workers = modules
      .map(({ label, alias, worker }) => worker && { label, alias, ...worker })
      .filter(Boolean);
...
function createLoaderRules(languages, features, workers, publicPath) {
  ...
  const workerPaths = workers.reduce(
    (acc, { label, output }) =>
      Object.assign(acc, {
        [label]: `${
          publicPath ? `${stripTrailingSlash(publicPath)}/` : ''
        }${output}`
      }),
    {}
  );
  const globals = {
    MonacoEnvironment: `((paths) => ({ getWorkerUrl: (moduleId, label) => paths[label] }))(${JSON.stringify(
      workerPaths,
      null,
      2
    )})`
...
function createPlugins(webpack, workers, outputPath) {
  const workerFallbacks = workers.reduce(
    (acc, { id, fallback }) =>
      fallback
        ? Object.assign(acc, {
  ...
  return [
    ...
    ...(workerFallbacks
      ? [createContextPlugin(webpack, WORKER_LOADER_PATH, workerFallbacks)]
      : [])
  ];
}
